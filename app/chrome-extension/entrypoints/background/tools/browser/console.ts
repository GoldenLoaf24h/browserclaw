...
class ConsoleTool extends BaseBrowserToolExecutor {
  ...
  async execute(args: ConsoleToolParams): Promise<ToolResult> {
    ...
    // Guard against popup.html
    if (url && isPopupUrl(url)) {
    ...
    try {
      if (typeof tabId === 'number') {
        ...
        // Navigate to the specified URL (background by default)
        targetTab = await this.navigateToUrl(url, background !== false, windowId);
      } else {
  ...
  private async navigateToUrl(
    ...
    if (isPopupUrl(url)) {
      throw new Error('Access denied: popup.html cannot be opened or navigated to by agent code paths.');
    }
    ...
    if (existingTabs.length > 0 && existingTabs[0]?.id) {
      ...
      if (background === false) {
        // Activate the existing tab only if background is explicitly false
        await chrome.tabs.update(tab.id!, { active: true });
        await chrome.windows.update(tab.windowId, { focused: true });
      }
      return tab;
    } else {
      // Create new tab with the URL (default active: false to protect user focus)
