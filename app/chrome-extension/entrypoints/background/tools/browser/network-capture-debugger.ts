...
// WebSocket capture: frames are high-volume (a busy page can emit thousands),
// so cap per connection and per frame. Text frames are kept verbatim up to
// MAX_WS_FRAME_CHARS; binary payloads are recorded by size only — the base64
// blob would dominate the capture and is rarely useful to an agent.
const MAX_WS_FRAMES_PER_CONNECTION = 200;
const MAX_WS_FRAME_CHARS = 4000;

interface WebSocketFrameInfo {
  requestId: string;
...
interface WebSocketConnectionInfo {
  ...
  frames: WebSocketFrameInfo[];
  framesTruncated?: boolean;
  closed?: boolean;
}

function wsFrameType(opcode: number): WebSocketFrameInfo['type'] {
  switch (opcode) {
    case 1:
      return 'text';
    case 2:
...
class NetworkDebuggerStartTool extends BaseBrowserToolExecutor {
  ...
  private handleDebuggerEvent(source: chrome.debugger.Debuggee, method: string, params?: any) {
    ...
    switch (method) {
      ...
      case 'Network.webSocketClosed':
        this.handleWebSocketClosed(tabId, params);
        break;
    }
  }

  /**
   * Register a WebSocket connection. The request that opened it is captured by
   * Network.requestWillBeSent, so this only records the URL and frame buffers.
   */
  private handleWebSocketCreated(tabId: number, params: any) {
    const captureInfo = this.captureData.get(tabId);
    ...
    if (!captureInfo.webSockets) captureInfo.webSockets = {};
    captureInfo.webSockets[requestId] = {
      requestId,
      url: url || '',
      sentCount: 0,
      receivedCount: 0,
  ...
  /** Record a single WebSocket frame (text kept, binary by size). */
  private handleWebSocketFrame(tabId: number, params: any, direction: 'sent' | 'received') {
    const captureInfo = this.captureData.get(tabId);
    if (!captureInfo || !captureInfo.webSockets) return;

    const { requestId, response, timestamp } = params || {};
    const connection: WebSocketConnectionInfo | undefined = captureInfo.webSockets[requestId];
    ...
    const frame: WebSocketFrameInfo = {
      requestId,
      direction,
      opcode,
      type: wsFrameType(opcode),
      timestamp: typeof timestamp === 'number' ? Math.round(timestamp * 1000) : Date.now(),
      payloadLength: rawPayload.length,
    };

    // Text frames are the useful ones; binary arrives base64-encoded and would
    // swamp the capture, so only its size is reported.
    if (opcode === 1 && rawPayload) {
      if (rawPayload.length > MAX_WS_FRAME_CHARS) {
        frame.payload = rawPayload.slice(0, MAX_WS_FRAME_CHARS);
        frame.truncated = true;
      } else {
        frame.payload = rawPayload;
      }
    }
    ...
    if (connection.frames.length < MAX_WS_FRAMES_PER_CONNECTION) {
  ...
  private handleWebSocketClosed(tabId: number, params: any) {
    const captureInfo = this.captureData.get(tabId);
    if (!captureInfo || !captureInfo.webSockets) return;

    const connection = captureInfo.webSockets[params?.requestId];
  ...
  async stopCapture(tabId: number, isAutoStop: boolean = false): Promise<any> {
    ...
    const resultData = {
      ...
      webSocketCount: webSocketConnections.length,
      webSocketFrameCount,
    ...
    try {
      if (targetUrl) {
        ...
        if (existingTabs.length > 0 && existingTabs[0]?.id) {
          ...
          // P0-1: Protect user focus
          if ((args as any).background === false) {
