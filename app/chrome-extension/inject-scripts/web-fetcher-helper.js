...
if (window.__WEB_FETCHER_HELPER_INITIALIZED__) {
  ...
  chrome.runtime.onMessage.addListener((request, _sender, sendResponse) => {
    const pingActions = ['chrome_web_fetcher_ping'];
    // Also answer the file-set keyed ping (mcp_ping_*) emitted by injectContentScript.
    if (String(request.action || '').startsWith('mcp_ping_')) {
      ...
      try {
        ...
        if (request.selector) {
          ...
          if (article && article.textContent) {
            ...
            // Combine content. Readability article.textContent flattens block
            // elements ("v4.12Qualification") which breaks downstream
            // parsing — re-derive text from the article HTML so block
            // boundaries survive as newlines.
            let fullContent = article.textContent;
            try {
              if (article.content) {
                const parsed = new DOMParser().parseFromString(article.content, "text/html");
                const derived = parsed.body.innerText || parsed.body.textContent;
                if (derived && derived.trim().length > 0) fullContent = derived;
              }
            } catch (fmtErr) {
              console.warn("textContent block-format derive failed:", fmtErr);
            }
