      ...
      if (waitForNavigation) {
        navigationPromise = new Promise((resolve) => {
          ...
          const finish = (result) => {
            if (resolved) return;
            resolved = true;
            window.removeEventListener('beforeunload', navListener);
            window.removeEventListener('pagehide', navListener);
            clearTimeout(timer);
            resolve(result);
          };
          const navListener = () => finish(true);
          window.addEventListener('beforeunload', navListener);
          window.addEventListener('pagehide', navListener);
