...
| **full**           |     50     |        ~19.5k tokens         | Complete canonical tool surface: raw CDP access, tab groups, diagnostics, performance |
...
- `action:back|forward` — Alternative to url="back"|"forward": navigate browser history backward or forward.
...
- `groupTitle` — Concise, task-aligned title for the Chrome tab group reflecting user intent in user language (e.g. "Best 4K Monitors on Amazon", "GitHub PR Review"). If omitted, the extension derives a smart title dynamically from the destination domain and page title instead of generic fallback.
...
- `dismissOverlays` — Automatically detect and dismiss visible marketing popups, coupon modals, and promotional overlays after navigation completes (default: false)
...
- `format:compact|html|fast` — Output format for treeString. "compact" (default) produces a concise, accessibility-tree-inspired representation without closing tags, slashing token usage by 60%+. "html" returns legacy pseudo-HTML tags. "fast" activates ultrafast atomic snapshot mode (10-30ms, <=15KB).
- `fast` — When true, activates the ultrafast atomic DOM snapshot engine (10-30ms, <=15KB payload) with WeakMap caching and native checkVisibility.
- `legacyVisibility` — When true, uses legacy visibility fallback (computedStyle display/visibility/opacity) instead of element.checkVisibility.
...
- `index` — 1-based element index from browserclaw_read_dom
...
Search the page without dumping full DOM tree. Supports searching interactive elements (returning indices for browserclaw_interact_index), all DOM nodes, or raw visible text lines.
...
- `autoScroll` — When true, automatically executes client-side RAF auto-scrolling to probe and detect off-screen virtualized elements (virtual list / infinite scroll streams like X/Twitter). Default: false
- `maxSteps` — Maximum scroll iterations to attempt when autoScroll is enabled (default: 10)
- `stepPx` — Pixel distance to scroll on each probing step (default: 800)
- `tabId` — Target tab ID (optional)
...
- `index` — Element numeric index from browserclaw_read_dom
...
Click, hover, or interact with an element using its compact 1-based numeric index from browserclaw_read_dom. When performing predictable multi-step actions (e.g. form submission or chain navigation), prefer browserclaw_batch_actions to finish in a single round-trip.
...
Fill text into an input or textarea element using its compact 1-based numeric index. For single search/form submission, pass pressEnter: true to fill and submit in 1 turn without needing a separate click. When filling multiple fields or clicking submit, use browserclaw_batch_actions to pipeline in 1 turn.
...
- `index` — Target element index (1-based integer from browserclaw_read_dom) to focus before sending keyboard events.
...
- `index` — Compact 1-based numeric index of the file input element from browserclaw_read_dom
- `clickTargetIndex` — Compact 1-based numeric index of a button/element from browserclaw_read_dom to click that triggers a dynamic file chooser dialog (e.g. Ant Design, Element Plus upload buttons) intercepted via CDP Page.setInterceptFileChooserDialog
...
- `index` — 1-based compact element index from browserclaw_read_dom targeting the rich-text editor or composer. Defaults to active element or discovered composer.
...
- `ref` — Element ref/index from browserclaw_read_dom. For click/scroll/scroll_to/key/type and drag end when provided; takes precedence over coordinates.
...
- `startRef` — Drag start ref/index from browserclaw_read_dom (alternative to startCoordinates).
...
- `tabId` — Target tab ID (optional)
- `windowId` — Target window ID (optional)
- `maxOverlays` — Maximum number of stacked overlays to dismiss (default: 5)
- `waitForSettle` — Wait for DOM settle after dismissal (default: true)
- `sessionId` — Session ID for tab affinity routing
- `sessionContext` — Session context alias

## Observation & Scrolling

### `chrome_screenshot`

[Prefer browserclaw_read_dom over taking a screenshot] Take a screenshot of the current page or a specific element. Returns base64 image directly in MCP image content block without writing to disk. By default, output is compressed JPEG with maxWidth <= 1280px. Debug disk save is available via savePng/saveToDisk into system temporary directory.
...
- `assetIndex` — View one visual asset listed by browserclaw_read_dom ([asset N] lines): returns the real image resource; falls back to a viewport crop when bytes are unavailable
...
- `targetIndex` — Compact 1-based numeric index of target element from browserclaw_read_dom to crop and capture only this specific region of interest
- `index` — Alias for targetIndex: compact 1-based numeric index of target element from browserclaw_read_dom to crop and capture
...
- `query` — Text string or regex pattern to search for during scrolling.
- `isRegex` — Whether to evaluate query as a regular expression (default: false).
- `selector` — CSS selector for the target element (supports Shadow DOM piercing combinators like >>>).
- `role` — ARIA role to filter matching elements (e.g. "button", "article", "link").
- `direction:down|up` — Scroll direction (default: "down").
- `stepPx` — Pixel offset to scroll on each step (default: 800).
- `maxSteps` — Maximum scroll iterations before stopping (default: 10, max: 30).
- `scrollDelayMs` — Pause duration in ms after each step to allow virtual list rendering (default: 250ms).
- `containerSelector` — Optional CSS selector of specific scrollable container instead of window/document.
- `actionOnFound:none|scroll_into_view|click|focus` — Action to perform automatically when element is found (default: "scroll_into_view").
- `tabId` — Target tab ID (optional).
- `windowId` — Target window ID (optional).
- `sessionId` — Session identifier for tab affinity (optional).
- `sessionContext` — Session context alias (optional).
