...
| 核心能力与架构维度 | **BrowserClaw (本项目)** | **browser-use** | **Playwright MCP (微软官方)** | **Stagehand** |
| :--- | :--- | :--- | :--- | :--- |
| **真实浏览器登录态复用** | ✅ **100% 原生继承**：无感复用 Google、GitHub 与企业 SSO | ⚠️ **配置繁琐**：独立新 Profile，频繁触发 2FA 与风控 | ❌ **空白沙盒**：每次启动纯净环境，无日常工作上下文 | ❌ **云端沙盒**：运行于远程容器，需手动注入 Cookie |
| **Windows 文件排他锁免疫** | ✅ **Native Messaging**：零文件锁冲突，不占端口静默运行 | ❌ **WinError 32**：复制用户目录必遭 Windows 独占锁崩溃 | ⚠️ **进程残留**：异常退出易残留无头 `chrome.exe` 占内存 | ⚠️ **纯云端**：网络延迟较高且有按次计费成本 |
| **单步交互 Token 成本** | ✅ **极度经济 (<800 Tokens)**：剪枝 DOM 索引 + 自驱局部 Diff | ⚠️ **3,000 - 8,000 Tokens**：全量 DOM 重算或调用视觉大模型 | ❌ **5,000 - 15,000 Tokens**：每步均 Dump 全量可访问性文本树 | ❌ **高开销**：单步语义指令依赖大模型重新推断目标 |
| **海量长页面检索** | ✅ **`chrome_grep` (<100 Tokens)**：毫秒级正则/文本定向秒搜 | ❌ **全量抓取后检索**：需将数万字 DOM 塞入上下文寻找 | ❌ **全量文本遍历**：Agent 需逐行遍历数万行无结构文本树 | ❌ **模糊推断**：依赖云端视觉或长提示词重新推断匹配 |
| **多步复合流水线** | ✅ **闭环 `batch_actions`**：单次网络往返完成连续操作、断言与提取 | ⚠️ **单步循环**：每步点击/打字均需大模型往返一次 (10s+) | ❌ **单步调用**：无内置批量流水线，无断言与字段提取 | ⚠️ **语义单步**：`page.act()` 每次调用独立计费与决策 |
| **真人体感视觉动效** | ✅ **1:1 弹簧物理光标**：顺滑飞行轨迹，专属色彩标签组隔离 | ❌ **无视觉层**：无头直接跳转，多标签页杂乱堆叠在主窗口 | ❌ **无视觉层**：纯为自动化测试设计，无用户防打扰或视觉反馈 | ⚠️ **远程投屏**：在网页控制台通过 Canvas 串流，非本地平滑层 |
| **2FA / 滑块人机协作** | ✅ **毛玻璃顶栏接管**：页面柔和暗化挂起，人类完成后一键恢复 | ❌ **超时崩溃**：遇到滑块验证码通常超时重试或任务死锁 | ❌ **测试报错**：无交互接管通道，流程直接抛出超时异常 | ⚠️ **控制台介入**：需切换至云端提供商后台网页手动点击 |
| **底层灵活穿透通道** | ✅ **`cdp_execute` + 防挂熔断**：多态路由，指令挂死自动 Detach | ⚠️ **Python CDP**：Python 异步直接派发 CDP，灵活性高 | ❌ **API 锁定**：仅暴露封装工具，无法自由穿透底层 CDP 协议 | ❌ **API 锁定**：仅限 Playwright 高层接口，无法发原生 CDP |
| **个人第二大脑记忆** | ✅ **历史与书签智理**：理解日常搜索偏好，自动整理知识库 | ❌ **无此功能**：一次性任务自动化，不沉淀浏览器长期记忆 | ❌ **无此功能**：自动化测试 runner，会话结束状态全销毁 | ❌ **无此功能**：云端临时会话，与开发者日常工作流割裂 |
