...
export async function testP01() {
  ...
  // 2. Token target: click AUTHORIZE button whose data-k === session.clickToken
  console.log('\n--- 2. Token target ---');
  ...
  // 3. Hover stack: SYSTEM -> DELTA -> RELEASE
  console.log('\n--- 3. Hover stack ---');
  dom = await callTool('chrome_read_dom', { tabId });
  ...
  dom = await callTool('chrome_read_dom', { tabId });
  const relEl = dom.indexedElements.find(e => e.attributes?.id === 'hover-release' || e.text?.includes('RELEASE'));
  ...
  console.log(`Clicking dynamic button [${dynEl.index}] (id="${dynEl.attributes?.id}")...`);
...
if (process.argv[1]?.endsWith('test-p01-clean.mjs')) {
  testP01().catch(err => {
    console.error(err);
    process.exit(1);
  });
